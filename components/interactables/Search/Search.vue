...
import { SearchQueryItem, SearchCommandList, SearchItemList, SearchResultItem, SearchCommandTypeParams, SearchCommandType, SearchCommand, CalendarDateType } from './Types'
...
export default Vue.extend({
  ...
  computed: {
    ...
    isDate() {
      ...
      if (this.current.command == SearchCommand.Before
  ...
  methods: {
    ...
    _insertOption() {
      ...
      let option = this.searchOptions[this.option]
      this.searchQuery.appendCommand(option.name, "")
      this.caretPosition += option.name.length + 1
      this.option = -1
      this.current = this.searchQuery.queryItems[this.searchQuery.queryItems.length - 1]
    },
    _insertHasOption() {
      if (this.option < 0 || this.current == null) {
        return
      }
      let option = this.hasOptions[this.option]
    ...
    _insertSearch() {
      ...
      let search = this.searchResult[this.search] as SearchResultItem
    ...
    _produceItems() {
      ...
      this.searchQuery.queryItems.forEach((queryItem : SearchQueryItem) => {
        if (queryItem.index > 0) {
          let blank : HTMLElement = document.createElement("span")
          blank.innerHTML = "&nbsp;"
          searchInput.appendChild(blank)
        }
        let newElement : HTMLElement = document.createElement("span")
    ...
    _setCaretPosition(position:number) {
      ...
      const searchInput = this.$refs.searchInput as HTMLElement
      this.searchQuery.queryItems.forEach((queryItem : SearchQueryItem) => {
        if (position >= queryItem.cursorStart && position <= queryItem.cursorEnd) {
          let el = searchInput.childNodes[queryItem.index * 2]
          let range = new Range()
          range.setStart(el.firstChild == undefined ? el : el.firstChild, position - queryItem.cursorStart)
      ...
      if (!hasSet) {
        let el = searchInput.lastChild
        if (el) {
          let range = new Range()
          range.setStart(el.firstChild == undefined ? el : el.firstChild, el.textContent ? el.textContent.length : 0)
          range.collapse(false)
          selection?.removeAllRanges()
          selection?.addRange(range)
        }
    ...
    _prepareSearch() {
      this.caretPosition = this._getCaretPosition()
      this.current = this.searchQuery.queryItemFrom(this.caretPosition) as SearchQueryItem
      ...
      this.searchResult = SearchItemList.filter((item) => {
        if (this.current != null) {  
          if (this.current.command != "") {
            return this.current.command.indexOf(item.command) == 0 && item.value.indexOf(this.current.value) == 0
          } else {
            return item.value.indexOf(this.current.value) == 0
          }
        }
