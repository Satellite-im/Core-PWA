  ...
  methods: {
    ...
    gotoAddFriends() {
      if (this.$device.isMobile) {
        this.$store.commit('ui/setSwiperSlideIndex', 2) // Add friend slide's index is 2
      }

      // if navigating from 'chat/direct' to 'friend' page and mobile
      if (this.$route.path.includes('/chat/direct') && this.$device.isMobile) {
        this.$store.commit('ui/showMobileAddFriend', true)
      }
