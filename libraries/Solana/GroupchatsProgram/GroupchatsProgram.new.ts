...
export default class GroupchatsProgram extends EventEmitter {
  ...
  async decryptInvite(invite: any) {
    const crypto = new Crypto()
    crypto.init(this._getPayer())
    const pass = await crypto.decryptFrom(invite.sender, invite.encryptionKey)
    return {
      ...invite,
      encryptionKey: pass,
      groupId: crypto.decryptWithPassword(invite.groupId, pass),
    }
  }

  generateEncryptionKey() {
    // TODO consider to change random key generation
    return base58(Keypair.generate().secretKey).slice(0, 64)
  }
  ...
  async invite(groupId: string, recipient: string) {
    ...
    const pass = this.generateEncryptionKey()
    const encryptedGroupId = await crypto.encryptWithPassword(groupId, pass)
    const encryptedPass = await crypto.encryptFor(user.toBase58(), pass)

    await program.rpc.invite(encryptedGroupId, user, encryptedPass, {
      accounts: {
