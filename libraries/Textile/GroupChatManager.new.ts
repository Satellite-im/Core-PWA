...
export class GroupChatManager {
  ...
  async getThreadName(): Promise<string> {
    ...
    return crypto
      .signMessageWithKey(this.textile.wallet.keypair.secretKey, `groupChats`)
      .toString()
  }
  ...
  async getThreadID(): Promise<ThreadID> {
    ...
    if (!thread) {
      return this.textile.client.newDB(undefined, name)
    }
  ...
  async getConversation({
    ...
    const threadID = ThreadID.fromString(groupThreadID)
